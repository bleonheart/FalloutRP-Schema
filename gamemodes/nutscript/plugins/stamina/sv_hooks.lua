...
function PLUGIN:PlayerLoadedChar(client)
	...
	timer.Create(uniqueID, 0.9, 0, function()
		...
		if (client:GetMoveType() != MOVETYPE_NOCLIP and client.character) then
			...
			local stamina = math_Clamp(client.character:GetVar("stamina", 300) + value, 0, 300)

			if (stamina != client.character:GetVar("stamina", 300)) then
...
function PLUGIN:PlayerSpawn(client)
	if (client.character) then
		client.character:SetVar("stamina", 300)
	end
end