...
local wheel = Material("forp/load_roulette_wheel.png", "noclamp smooth")
local ball = Material("forp/load_roulette_ball.png", "noclamp smooth")
local bars = Material("forp/load_roulette_bars.png")

nut.loadAlpha = nut.loadAlpha or 0
nut.loadScreenAlpha = nut.loadScreenAlpha or 255
nut.loadScreenWheel = nut.loadScreenWheel or 0
nut.loadScreenBall = nut.loadScreenBall or 0
	...
	if (!gui.IsGameUIVisible() and nut.loadScreenAlpha > 0) then
		...
		if (nut.loadAlpha == 255 and goal == 255) then
			if (nut.loadScreenAlpha == 255) then
				...
				nut.gui.charMenu:FadeIn()
			end

			nut.loadScreenAlpha = math.Approach(nut.loadScreenAlpha, 0, FrameTime() * 600)
		end

		nut.loadScreenWheel = math.Approach(nut.loadScreenWheel, math.huge, FrameTime() * 45)
		nut.loadScreenBall = math.Approach(nut.loadScreenBall, math.huge, FrameTime() * 60)

		local alpha = nut.loadScreenAlpha
		local wheelrot = nut.loadScreenWheel
		local ballrot = nut.loadScreenBall

		if (alpha > 0) then
			SetDrawColor(20, 20, 20, alpha)
			DrawRect(0, 0, scrW, scrH)

			local x, y, w, h = scrW*0.5 - 128, scrH*0.4 - 128, 256, 256

			surface.SetDrawColor(Color(255, 194, 0, alpha))
			--surface.SetDrawColor(Color(255, 255, 255, alpha))

			surface.SetMaterial(bars)
			surface.DrawTexturedRectRotated(x + 348, y + 128, w, h, 0)
			surface.DrawTexturedRectRotated(x - 92, y + 128, w, h, 180)

			surface.SetMaterial(wheel)
			surface.DrawTexturedRectRotated(x + 128, y + 128, w, h, wheelrot)

			surface.SetMaterial(ball)
			surface.DrawTexturedRectRotated(x + 128, y + 128, w, h, -ballrot)

			for i = #nut.loadingText, 1, -1 do
				local alpha2 = (1-i / #nut.loadingText) * alpha

				draw.SimpleText(nut.loadingText[i], "Monofonto24", scrW * 0.5, scrH * 0.6 + (i * 36), Color(255-40, 194-40, 0, alpha2), 1, 1)
			end
...
local introText = table.Random({"Burn away the flags. Begin again.", 
...
function GM:HUDPaint()
	...
	if (nut.fadeStart and nut.fadeFinish) then
		...
		draw.SimpleText(introText, "nut_TargetFont", scrW * 0.5, (scrH * 0.35) + h, color, 1, 1)
...
function GM:CreateSideMenu(menu)
	...
	if (nut.config.showMoney and nut.currency.IsSet()) then
		...
		menu.money.Think = function(label)
			label:SetText(string.Comma(LocalPlayer():GetMoney()).." Caps")
		end
...
function GM:HUDPaintTargetPlayer(client, x, y, alpha)
	...
	if (client:Health() < 75) then
		...
		local statcolor = Color(math.min(255 + add, 255), math.min(25 + add, 255), math.min(30 + add, 255))

		if (client:Health() < 25) then
			nut.util.DrawText(x, y, "This character appears to be in critical condition.", statcolor, "nut_TargetFontSmall")
			y = y + nut.config.targetTall
		elseif (client:Health() < 50) then
			nut.util.DrawText(x, y, "This character appears to be injured.", statcolor, "nut_TargetFontSmall")
			y = y + nut.config.targetTall
		elseif (client:Health() < 75) then
			nut.util.DrawText(x, y, "This character appears to be hurt.", statcolor, "nut_TargetFontSmall")
			y = y + nut.config.targetTall
		end
	end
