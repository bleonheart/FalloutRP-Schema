...
do
	...
	if (SERVER) then
		...
		function playerMeta:GiveMoney(amount)
			self:SendNotification("You\'ve gained " .. amount .. " Caps", "", "", "fosounds/fix/ui_items_bottlecaps_up_0"..math.random(1,4)..".mp3", 15, 0)
			self:SetMoney(self:GetMoney() + amount)
		end

		-- Takes away a certain amount by inverting the amount specified.
		function playerMeta:TakeMoney(amount)
			self:SendNotification("You\'ve lost " .. amount .. " " .. "Caps", "", "", "fosounds/fix/ui_items_bottlecaps_up_0"..math.random(1,4)..".mp3", 12, 0)
			self:SetMoney(self:GetMoney() - amount)
		end
	end
end
