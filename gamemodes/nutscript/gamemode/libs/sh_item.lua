	...
	if (isBase) then
		...
		itemTable.functions.Drop = {
			...
			run = function(itemTable, client, data)
				if (SERVER) then
					if (itemTable.CanTransfer and itemTable:CanTransfer(client, data) == false or data.Equipped) then
						return false
					end
		...
		itemTable.functions.Take = {
			...
			run = function(item)
				if (SERVER) then
					...
					item.player:SendSound(itemTable.takeSound or "fosounds/fix/ui_items_generic_up_0"..math.random(1,4)..".mp3")
		...
		itemTable.functions.Inspect = {
			entityOnly = true,
			tip = "Inspect the item.",
			icon = "icon16/magnifier.png",
			run = function(item)
				if (CLIENT) then
					SCHEMA:InspectItem(item)
				end
				return false
			end,
			shouldDisplay = function(itemTable, data, entity)
		...
		itemTable.functions.Modify = {
			menuOnly = true,
			tip = "Modify the Item.",
			icon = "icon16/cut.png",
			run = function(item, client, data, entity, index)
				if (CLIENT) then
					SCHEMA:ModifyItem(item, index)
				end
		...
		itemTable.functions.SellToMenu = {
			text = "Sell Item",
			tip = "Sell the item.",
			icon = "icon16/money.png",
			run = function(itemTable, client, data)
				if not client:HasFlag("3") then 
					return false 
				end
				if (itemTable.CanTransfer and itemTable:CanTransfer(client, data) == false or data.Equipped) then
					return false
				...
				if SERVER then
					...
					if data.custom and data.custom.condition then
						condition = data.custom.condition
					end
					condition = condition / 100
					price = math.Clamp(itemTable.price * condition, itemTable.price * 0.1, itemTable.price) * 0.5
					client:GiveMoney(price)
				end
				return true
			end,
			shouldDisplay = function(itemTable, data, entity)
				return LocalPlayer():HasFlag("3")
			end,
		}
		...
		if (!itemTable.GetWeight) then
			function itemTable:GetWeight(data)
				if (data) then
					return data.weight or itemTable.weight
				else
					return itemTable.weight
				end
			end
		end
...
do
	...
	if (SERVER) then
		...
		function nut.item.Spawn(position, angles, itemTable, data, client)
			...
			entity:Spawn()
			entity:SetModel(data and data.custom and data.custom.model or itemTable.GetDropModel and itemTable:GetDropModel() or itemTable.model)
	...
	function playerMeta:GetInvWeight(inventory)
		...
		local additionalCarry = 0

		for itemID, items in pairs(inventory or self:GetInventory()) do
			local itemTable = nut.item.Get(itemID)

			for k, item in pairs(items) do
				if item.data.Equipped and itemTable.CarryWeight then
					additionalCarry = additionalCarry + itemTable.CarryWeight
				end
			end
		end

		maxWeight = maxWeight + additionalCarry
