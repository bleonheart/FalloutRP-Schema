...
if (SERVER) then
	...
	netstream.Hook("nut_CharCreate", function(client, data)
		...
		Data["skintone"] = data.skintone
		Data["facemap"] = data.facemap
		Data["eyecolor"] = data.eyecolor
		Data["hair"] = data.hair
		Data["haircolor"] = data.haircolor
		Data["facialhair"] = data.facialhair

		local extraData = {}
		extraData = data.facemap
		extraData = data.eyecolor
		extraData = data.hair
		extraData = data.haircolor
		extraData = data.facialhair
		...
		local clt = math.random(1, 14)
		inventory:Add("Farm Hand Outfit ("..clt..")", 1, nut.item.Get("Farm Hand Outfit ("..clt..")").data)
		...
		nut.char.Create(client, charData, function(id)
			...
			hook.Run("PlayerCreatedChar", client, charData, extraData)
	...
	netstream.Hook("nut_CharChoose", function(client, index)
		client.lastchoose = client.lastchoose or 0
		
		if (CurTime() > client.lastchoose) then
			client.lastchoose = CurTime() + 60
		end
