...
function PLAYER:GetHandsModel()
	...
	local Skin = client:GetSkinTone()

	if type(Skin) == "string" then
		Skin = 0
	end

	return { model = "models/thespireroleplay/humans/group100/arms.mdl", skin = Skin, body = "0000000" }
end
...
do
	...
	if (SERVER) then
		...
		function playerMeta:ForceRagdoll()
			...
			if SCHEMA.ClothesTable[self] then
				for k, v in pairs(SCHEMA.ClothesTable[self]) do
					local model = ""
					if type(v.model) == "table" then
						model = v.model[self:GetGender()]
					elseif (v.Type == "Arms") then
						model = v.model..self:GetGender().."_arm"..".mdl"
					else
						model = v.model..self:GetGender()..".mdl"
					end
					v.entity = ents.Create("base_gmodentity")
					v.entity:SetModel(model)
					v.entity:SetPos(self.ragdoll:GetPos())
					if (v.bodygroup) then
					...
					v.entity:SetParent(self.ragdoll)
					v.entity:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL,EF_PARENT_ANIMATES))
					v.entity:PhysicsInit(SOLID_VPHYSICS)
					v.entity:SetMoveType(MOVETYPE_NONE)
					v.entity:SetSolid(SOLID_NONE)
