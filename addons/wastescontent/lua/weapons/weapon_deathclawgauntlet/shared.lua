...
if ( CLIENT ) then
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/deathclawgauntlet.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(5.658, -2.093, 1.646), angle = Angle(-0.602, -89.066, 7.826), size = Vector(1.003, 1.003, 1.003), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/deathclawgauntlet.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(7.361, 1.845, -0.378), angle = Angle(180, -90.001, 0.294), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	SWEP.Slot			= 0					 
	SWEP.SlotPos		= 1
	SWEP.DrawAmmo		= false					 
...
SWEP.Category			= "Fallout Sweps - Unarmed Weapons"
...
SWEP.HoldType = "fist"
SWEP.ViewModelFOV = 62.755905511811
SWEP.ViewModelFlip = false
SWEP.ViewModel = "models/weapons/v_stunbaton.mdl"
SWEP.WorldModel = "models/Halokiller38/fallout/weapons/Melee/deathclawgauntlet.mdl"
SWEP.ShowViewModel = true
SWEP.ShowWorldModel = true
SWEP.ViewModelBoneMods = {
	["Bip01 R Finger1"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-0.031, 49.715, 0) },
	["Bip01 R Finger2"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 50.744, 0) },
	["Dummy14"] = { scale = Vector(0.009, 0.009, 0.009), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) },
	["Bip01 R Finger21"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 52.659, 0) },
	["Bip01 R Finger0"] = { scale = Vector(1, 1, 1), pos = Vector(0, -0.264, 0), angle = Angle(2.543, -28.511, 0) },
	["Bip01 R Finger11"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 52.734, 0) },
	["Bip01 R Finger41"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 52.659, 0) },
	["Bip01 R Finger3"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 50.744, 0) },
	["Bip01 R Finger4"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 50.744, 0) },
	["Bip01"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-27.767, 22.02, -20.107) },
	["Bip01 R Finger31"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 52.659, 0) }
}
...
SWEP.WallSound 				= Sound("weapons/bat_baseball_hit_world1.wav")
...
function SWEP:PrimaryAttack()
	...
	if (!IsValid(trace.Entity)) then 
		...
		tr.endpos = self.Owner:GetShootPos() + (self.Owner:GetAimVector() * reach)
		tr.filter = self.Owner
		tr.mins = Vector(-13, -13, -10)
		tr.maxs = Vector(13,13,10)
		tr.mask = MASK_SHOT
		trace = util.TraceHull(tr)
	end

	self.Weapon:SetNextPrimaryFire(CurTime() + self.Primary.Delay + math.random(-10,10)/100)
	...
	if ( trace.Hit ) then
		...
		if (trace.Entity:IsPlayer() or trace.Entity:IsNPC() or string.find(trace.Entity:GetClass(),"prop_ragdoll") or (trace.Entity:GetClass() == "prop_door_rotating")) then
			if (SERVER) then 
				...
				dmginfo:SetDamage(math.random(self.Primary.Damage-5,self.Primary.Damage+5))
			...
			util.Effect("ImpactJeep",effectData,true)			
		end
	else
