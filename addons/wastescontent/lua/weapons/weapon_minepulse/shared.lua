...
SWEP.Gun = ("weapon_minepulse") -- must be the name of your swep but NO CAPITALS!
...
SWEP.PrintName				= "Pulse Mine"		-- Weapon name (Shown on HUD)	
...
SWEP.WorldModel = "models/halokiller38/fallout/weapons/mines/pulsemine.mdl"
...
SWEP.Primary.Ammo			= "PulseMine"				
-- pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, AirboatGun
-- Pistol, buckshot, and slam always ricochet. Use AirboatGun for a metal peircing shotgun slug

SWEP.Primary.Round 			= ("weapon_minepulse")	--NAME OF ENTITY GOES HERE
...
SWEP.VElements = {
	["mine"] = { type = "Model", model = "models/halokiller38/fallout/weapons/mines/pulsemine.mdl", bone = "Bip01_R_Hand", rel = "", pos = Vector(1.917, 5.41, 2.246), angle = Angle(180, 38.566, 3.615), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["mine"] = { type = "Model", model = "models/halokiller38/fallout/weapons/mines/pulsemine.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.68, 6.245, -3.605), angle = Angle(-12.221, -149.124, 19.67), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	...
	timer.Simple(plant, function() if not IsValid(self) then return end if IsValid(self.Owner) and IsValid(self.Weapon) then
		if self.Owner:Alive() and self.Owner:GetActiveWeapon():GetClass() == self.Gun then
			self.Weapon:SendWeaponAnim(ACT_VM_SECONDARYATTACK)
				...
				proxy = ents.Create("m9k_pulse")
				proxy:SetPos(trace.HitPos + trace.HitNormal + Vector(0,0,3)) //adjusted the height of the position so it does not spawn in ground
				trace.HitNormal.z = -trace.HitNormal.z
				proxy:SetAngles(trace.HitNormal:Angle() - Angle(180, 180, 0)) // Changed the X angle so it spawned normal to the ground
