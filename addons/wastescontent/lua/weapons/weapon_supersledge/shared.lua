...
if ( CLIENT ) then
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/supersledge.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(3.536, -2.277, -11.804), angle = Angle(-7.768, -75.276, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/supersledge.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.798, 1.957, 13.668), angle = Angle(180, -109.561, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	SWEP.Slot			= 0					 
	SWEP.SlotPos		= 1
	SWEP.DrawAmmo		= false					 
...
SWEP.Category			= "Fallout Sweps - Blunt Weapons"
...
SWEP.WorldModel = "models/Halokiller38/fallout/weapons/Melee/supersledge.mdl"
...
SWEP.MissSound 				= Sound("weapons/props_dane/generic/large/swing0.wav")
SWEP.WallSound 				= Sound("weapons/props_dane/generic/large/impact0.wav")
...
function SWEP:PrimaryAttack()
	...
	if ( trace.Hit ) then
		...
		if (trace.Entity:IsPlayer() or trace.Entity:IsNPC() or string.find(trace.Entity:GetClass(),"prop_ragdoll") or (trace.Entity:GetClass() == "prop_door_rotating")) then
			self.Weapon:SetNextPrimaryFire(CurTime() + self.Primary.Delay -.75)
			if (SERVER) then 
			...
			if (trace.Entity:GetClass() ~= "prop_door_rotating") then 
				self:EmitSound("weapons/props_dane/generic/large/impact1.wav")
				local effectData = EffectData()
				effectData:SetOrigin(trace.HitPos-trace.HitNormal*25)
				effectData:SetRadius(1)
				effectData:SetNormal(trace.HitNormal)
				util.Effect("BloodImpact",effectData)
				util.Decal("Blood",trace.HitPos + trace.HitNormal*40,trace.HitPos-trace.HitNormal*40)
				bullet = {}
				bullet.Num    = 1
			...
			util.Decal("ManhackCut",trace.HitPos + trace.HitNormal*20,trace.HitPos-trace.HitNormal*20)
		end
	else
