/**
* Scripted weapon based on Fallout: New Vegas' Euclid's C-Finder.
*
* @author Kyle James
* @version 20 August 2019
*/

SWEP.PrintName = "Euclid's C-Finder"
    
SWEP.Author = "Darkwater Entertainment"
SWEP.Contact = ""
SWEP.Purpose = "The Archimedes II orbital laser releases a huge amount of heat energy, capable of reducing many foes to ashes."
SWEP.Instructions = "Left click on a destination for an orbital strike to hit. "
...
SWEP.WorldModel = "models/halokiller38/fallout/weapons/energy weapons/euclidfinder.mdl"
...
SWEP.VElements = {
	["pistol"] = { type = "Model", model = "models/halokiller38/fallout/weapons/energy weapons/euclidfinder.mdl", bone = "v_weapon.USP_Parent", rel = "", pos = Vector(-0.068, 2.187, 5.105), angle = Angle(0, 0, -84.647), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["pistol"] = { type = "Model", model = "models/halokiller38/fallout/weapons/energy weapons/euclidfinder.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(0.856, 1.2, 2.296), angle = Angle(-4.357, 90, -173.78), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Primary.Sound = Sound("weapon_cfinder1.ogg") 
...
function SWEP:Think()
    ...
    if self:GetFakeTimer() < CurTime() && self:Clip1() < 1  then
        self:SetFakeTimer(self:GetFakeTimer() + 10)
	...
	if !self.Owner:KeyDown(IN_SPEED) and !self.Owner:KeyDown(IN_RELOAD) then
		...
		if skycheck.HitSky then
			if SERVER then
				...
				Satellite = ents.Create("fswp_archimedes2")
