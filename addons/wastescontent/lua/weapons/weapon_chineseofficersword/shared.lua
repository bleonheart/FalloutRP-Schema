...
if ( CLIENT ) then
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/chineseofficersword.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(4.073, -1.099, -1.32), angle = Angle(4.218, 105.4, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/chineseofficersword.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.801, 1.197, 2.566), angle = Angle(180, 59.54, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	SWEP.Slot			= 0					 
	SWEP.SlotPos		= 1
	SWEP.DrawAmmo		= false					 
...
SWEP.Category			= "Fallout Sweps - Bladed Weapons"
...
SWEP.WorldModel = "models/Halokiller38/fallout/weapons/Melee/chineseofficersword.mdl"
...
SWEP.MissSound 				= Sound("weapons/props_dane/generic/small/swing0.wav")
SWEP.WallSound 				= Sound("weapons/props_dane/combatknife/hit_other0.wav")
...
function SWEP:PrimaryAttack()
	...
	if ( trace.Hit ) then
		...
		if (trace.Entity:IsPlayer() or trace.Entity:IsNPC() or string.find(trace.Entity:GetClass(),"prop_ragdoll") or (trace.Entity:GetClass() == "prop_door_rotating")) then
			self.Weapon:SetNextPrimaryFire(CurTime() + self.Primary.Delay -.4)
			if (SERVER) then 
			...
			if (trace.Entity:GetClass() ~= "prop_door_rotating") then 
				self:EmitSound("weapons/blade_hit"..math.random( 1, 4 )..".wav")
