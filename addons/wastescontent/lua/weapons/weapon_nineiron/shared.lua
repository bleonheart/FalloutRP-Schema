...
if ( CLIENT ) then
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/9iron.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(4.091, -0.456, -5), angle = Angle(4.218, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/9iron.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.801, 1.171, 7.155), angle = Angle(180, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	SWEP.Slot			= 0					 
	SWEP.SlotPos		= 1
	SWEP.DrawAmmo		= false					 
...
SWEP.WorldModel = "models/Halokiller38/fallout/weapons/Melee/9iron.mdl"
...
SWEP.WallSound 				= Sound("weapons/bat_baseball_hit_world1.wav",100,200)
	...
	if ( trace.Hit ) then
		...
		if (trace.Entity:IsPlayer() or trace.Entity:IsNPC() or string.find(trace.Entity:GetClass(),"prop_ragdoll") or (trace.Entity:GetClass() == "prop_door_rotating")) then
			...
			if (trace.Entity:GetClass() ~= "prop_door_rotating") then 
				self:EmitSound("weapons/bat_baseball_hit_flesh.wav",100,math.random(150,200))
			...
			effectData:SetRadius(.1)
			effectData:SetNormal(trace.HitNormal)
			util.Effect("ManhackSparks",effectData,true)		
			util.Effect("ImpactJeep",effectData,true)
		end
	else
		self.Weapon:EmitSound(self.MissSound,100,math.random(150,255))
